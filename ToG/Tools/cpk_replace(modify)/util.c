...
void write_64_be(uint64_t value, unsigned char bytes[8])
{
    for (int i=7; i>=0; i--, value >>= 8) bytes[i] = value & 0xff;
}
...
void replace_data(FILE *outfile, long offset, unsigned char *bytes, size_t size)
{
    CHECK_ERRNO(fseek(outfile, offset, SEEK_SET) != 0, "fseek");
    CHECK_ERRNO(fwrite(bytes, 1, size, outfile) != size, "fwrite");
}